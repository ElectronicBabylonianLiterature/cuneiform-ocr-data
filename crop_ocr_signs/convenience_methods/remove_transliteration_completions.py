...
"""
Remove completions from transliteration of fragments.

Method not ultimately needed for determining if a cropped sign has partial ordering (see verify_signs.get_partial_order_signs). 
...
def read_fragment_text_object():
    with open("fragment_text_object.json", "r", encoding="utf-8") as f:
        data = json.load(f)
        return data
